...
lotkavolterra(a, b, c, g, dt, x_0, y_0) =    
    prey_level(out * x) , 
    pred_level(out * y)
    letrec {
        'x = reset * (x_0 + x + dt * (a * x - b * x * y));
        'y = reset * (y_0 + y + dt * (g * x * y - c * y));
    };
...
//process(x1, x2) = lotkavolterra(a, b, c, g, dt, input(x1), input(x2));
process = lotkavolterra(4, 1, 2, 1, .1, 1-1', 1-1');
